...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    const trojanURL = `trojan://${UUID}@www.visa.com.tw:443?security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${NAME}-${ISP}`;
...
function handleTrojanConnection(ws, msg) {
  try {
    ...
    const possiblePasswords = UUID.replace(/-/g, '').toUpperCase();
